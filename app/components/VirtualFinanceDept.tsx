        ...
        <div className="grid md:grid-cols-[1fr_1.5fr] gap-10 md:gap-20 mb-14 md:mb-16 items-end">
          <div>
            ...
            <h2 className="display text-[32px] sm:text-[40px] md:text-[48px] font-semibold text-[var(--text-primary)] leading-[1.04]">
              Finance that <span className="accent">scales</span><br />
              with you.
            </h2>
          </div>
          <p className="text-[16px] md:text-[17px] leading-[1.7] text-[var(--text-secondary)] md:pb-1">
            Accountables combines operational accounting, reporting, compliance and finance coordination within a connected operating environment — designed for growing businesses.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
          {pillars.map((p) => (
            <Link
              key={p.number}
              href={p.href}
              className="card p-7 md:p-8 flex flex-col gap-5 group"
            >
              {/* Icon + number */}
